...
entity pwm_led is
generic (
        WIDTH : positive := 10 -- RGB LEDs, and LEDs
        );
port (
     clk   : in  std_logic;
     rst   : in  std_logic;
...
-- Constants
--constant PWM_PERIOD       : integer := 1000000; -- 2,500,000 clock cycles to complete one cycle 
constant PWM_PERIOD       : integer := 10; -- PWM_freq = clk_freq / pwm_period = 125 MHz / 10 = 12.5MHz  
...
begin
PWM_Counter_Process : process (clk, rst) begin
...
led(WIDTH-1 downto 0) <= (others => pwm_out);
probe                 <= pwm_out;
end Behavioral;